...
function dogsFeeding(dogs) {
  dogs.forEach((dog) => {
    const recommendedFood = Math.trunc(dog.weight ** 0.75 * 28);
    dog["recFoodPortion"] = recommendedFood;
  });
  ...
  console.log(
    sarahDog.curFood > sarahDog.recFoodPortion
  ...
  const dogsOkayAmountOfFood = dogs.filter(
    (dog) =>
      dog.curFood >= dog.recFoodPortion * 0.9 &&
      dog.curFood <= dog.recFoodPortion * 1.1
  );

  console.log(dogsOkayAmountOfFood);
